...
export const AppLayout: React.FC<AppLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F17] flex flex-col md:flex-row text-[#F9FAFB]">
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-[#232D3F] bg-[#161B26]/80 backdrop-blur-xl p-5 justify-between sticky top-0 h-screen z-30">
        <div>
          {/* Logo & Brand Header */}
        ...
        <div className="border-t border-[#232D3F] pt-4 space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-medium text-[#9CA3AF] hover:text-red-400 hover:bg-red-500/10 transition-colors"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 border-b border-[#232D3F] bg-[#161B26]/60 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-[#6366F1]/10 text-[#6366F1] border border-[#6366F1]/20 flex items-center gap-1.5">
              <Zap className="w-3 h-3" /> Grounded RAG Engine Active
            </span>
          </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#161B26] border-t border-[#232D3F] flex items-center justify-around z-40 px-2">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = location.pathname === item.path;
            return (
